...
class MobileGPT:
    ...
    def __init__(self, socket):
        ...
        self.explored_subtasks = {}  # 페이지별 시도한 서브태스크 추적 {page_index: [subtask_names]}
        self.explored_uis = {}       # 페이지별 시도한 UI 인덱스 추적 {page_index: [ui_indexes]}
    ...
    def get_next_action(self, parsed_xml=None, hierarchy_xml=None, encoded_xml=None):
        ...
        if len(new_subtasks) > 0:
            available_subtasks += new_subtasks  # 새로 발견된 서브태스크 추가
    ...
    def init_explore(self, app_name: str, algorithm: str = "DFS"):
        ...
        self.visited_pages = set()                   # 방문한 페이지 집합
        ...
        self.unexplored_subtasks = {}                # Greedy용: {page_index: [subtask_info, ...]}
    ...
    def get_explore_action(self, parsed_xml, hierarchy_xml, encoded_xml, page_index):
        ...
        # 새로운 페이지 방문 시 초기화
        if page_index not in self.visited_pages:
            self._register_new_page(page_index, encoded_xml)

        # 알고리즘에 따라 다음 액션 결정
        if self.exploration_algorithm == "DFS":
            return self._get_dfs_action(parsed_xml, encoded_xml, page_index)
        elif self.exploration_algorithm == "BFS":
            return self._get_bfs_action(parsed_xml, encoded_xml, page_index)
        elif self.exploration_algorithm == "GREEDY_BFS":
            return self._get_greedy_bfs_action(parsed_xml, encoded_xml, page_index)
        elif self.exploration_algorithm == "GREEDY_DFS":
            return self._get_greedy_dfs_action(parsed_xml, encoded_xml, page_index)
        else:
    ...
    def _load_unexplored_subtasks(self):
        """available_subtasks.csv에서 unexplored 서브태스크 로드 (모든 trigger UI별로)"""
        pages_dir = self.memory.page_database_path
        if not os.path.exists(pages_dir):
            return
            ...
            if os.path.exists(available_file):
                try:
                    ...
                    if not unexplored_df.empty:
                        ...
                        if subtask_infos:
                            self.unexplored_subtasks[page_index] = subtask_infos
                except Exception as e:
                    log(f"Error reading available_subtasks.csv for page {page_folder}: {e}", "yellow")
    ...
    def _register_new_page(self, page_index, encoded_xml):
        """새 페이지 방문 시 unexplored 서브태스크를 스택/큐에 추가 (서브태스크 기반)"""
        self.visited_pages.add(page_index)
        self.explored_uis[page_index] = []

        # available_subtasks.csv에서 unexplored 서브태스크 가져오기
        ...
        log(f"Page {page_index}: found {len(unexplored_subtasks)} unexplored subtasks", "cyan")
        ...
        log(f"Page {page_index}: {len(subtask_infos)} subtask-trigger combinations: {[(s['name'], s['trigger_ui_index']) for s in subtask_infos]}", "cyan")

        # 알고리즘에 따라 스택/큐에 추가
        if self.exploration_algorithm == "DFS":
            for subtask_info in reversed(subtask_infos):
                self.exploration_stack.append((page_index, subtask_info))
        elif self.exploration_algorithm == "BFS":
            for subtask_info in subtask_infos:
                self.exploration_queue.append((page_index, subtask_info))
        elif self.exploration_algorithm in ["GREEDY_BFS", "GREEDY_DFS"]:
            # GREEDY_BFS/GREEDY_DFS: unexplored_subtasks 딕셔너리에 서브태스크 정보 추가
            if subtask_infos:
                self.unexplored_subtasks[page_index] = subtask_infos
                log(f"Page {page_index}: added to unexplored_subtasks", "cyan")

    def _get_dfs_action(self, parsed_xml, encoded_xml, current_page_index):
        """DFS: 스택 기반 깊이 우선 탐색 (Multi-step 서브태스크 지원)"""

        # 현재 진행 중인 Multi-step 서브태스크가 있으면 계속 진행
        if self.current_exploring_subtask is not None:
            return self._continue_subtask_exploration(parsed_xml, encoded_xml, current_page_index)

        while self.exploration_stack:
            page_index, subtask_info = self.exploration_stack[-1]
            ...
            if (subtask_name, trigger_ui_index) in self.explored_subtasks.get(page_index, []):
                continue

            # 새 서브태스크 탐색 시작
            log(f"DFS: Starting multi-step exploration of subtask '{subtask_name}' (trigger_ui={trigger_ui_index}) on page {current_page_index}", "cyan")
            return self._start_subtask_exploration(parsed_xml, encoded_xml, current_page_index, subtask_info)
    ...
    def _start_subtask_exploration(self, parsed_xml, encoded_xml, current_page_index, subtask_info):
        ...
        # explored_subtasks에 (subtask_name, trigger_ui_index) 튜플로 기록
        if current_page_index not in self.explored_subtasks:
            self.explored_subtasks[current_page_index] = []
        self.explored_subtasks[current_page_index].append((subtask_name, trigger_ui_index))

        # explored_uis에도 기록
        if trigger_ui_index is not None and trigger_ui_index != -1:
            if current_page_index not in self.explored_uis:
                self.explored_uis[current_page_index] = []
            self.explored_uis[current_page_index].append(trigger_ui_index)

        # 첫 번째 액션: trigger UI 클릭
        action = self._create_click_action(parsed_xml, encoded_xml, trigger_ui_index, current_page_index)
        if action:
        ...
        self._finish_subtask_exploration(current_page_index, success=False)
        return None

    def _continue_subtask_exploration(self, parsed_xml, encoded_xml, current_page_index):
    ...
    def _finish_subtask_exploration(self, end_page_index, success=True):
        ...
        log(f"Finishing subtask '{subtask_name}' (trigger_ui={trigger_ui_index}): {len(actions)} actions, start={start_page}, end={end_page_index}", "blue")
        ...
        # 메모리에 저장 (mark_subtask_explored_multistep 호출)
    ...
    def _get_bfs_action(self, parsed_xml, encoded_xml, current_page_index):
        ...
        while self.exploration_queue:
            target_page, subtask_info = self.exploration_queue[0]
            ...
            if target_page != current_page_index:
                ...
                if path:
                    log(f"BFS: Found path from page {current_page_index} to page {target_page}: {path}", "cyan")
                    self.navigation_plan = path
                    return self._execute_navigation_step(parsed_xml, encoded_xml, current_page_index)
                else:
                    log(f"BFS: No path found from page {current_page_index} to page {target_page}, skipping", "yellow")
            ...
            subtask_name = subtask_info['name']
            trigger_ui_index = subtask_info.get('trigger_ui_index', -1)
            if (subtask_name, trigger_ui_index) in self.explored_subtasks.get(target_page, []):
                continue

            # 새 서브태스크 탐색 시작
            log(f"BFS: Starting multi-step exploration of subtask '{subtask_name}' (trigger_ui={trigger_ui_index}) on page {current_page_index}", "cyan")
            return self._start_subtask_exploration(parsed_xml, encoded_xml, current_page_index, subtask_info)
        ...
        # 이미 해당 페이지에 있으면 재귀 호출로 queue 처리
        return self._get_bfs_action(parsed_xml, encoded_xml, current_page_index)

    def _get_greedy_bfs_action(self, parsed_xml, encoded_xml, current_page_index):
        ...
        target_page, target_subtask_info, path = self._find_nearest_unexplored_info(current_page_index)
        ...
        log(f"Greedy-BFS: Nearest unexplored is '{subtask_name}' on page {target_page}", "cyan")

        # 현재 페이지가 아니면 네비게이션 필요
        if path:
        ...
        # unexplored 리스트에서 제거
        if current_page_index in self.unexplored_subtasks:
            self.unexplored_subtasks[current_page_index] = [
                s for s in self.unexplored_subtasks[current_page_index]
                if s['name'] != subtask_name
            ]

        # 새 서브태스크 탐색 시작
        log(f"Greedy-BFS: Starting multi-step exploration of subtask '{subtask_name}' on page {current_page_index}", "cyan")
        return self._start_subtask_exploration(parsed_xml, encoded_xml, current_page_index, target_subtask_info)

    def _get_greedy_dfs_action(self, parsed_xml, encoded_xml, current_page_index):
        ...
        target_page, target_subtask_info, path = self._find_deepest_unexplored_info(current_page_index)
        ...
        subtask_name = target_subtask_info['name']
        log(f"Greedy-DFS: Deepest unexplored is '{subtask_name}' on page {target_page}", "cyan")
        ...
        # 새 서브태스크 탐색 시작
        log(f"Greedy-DFS: Starting multi-step exploration of subtask '{subtask_name}' on page {current_page_index}", "cyan")
        return self._start_subtask_exploration(parsed_xml, encoded_xml, current_page_index, target_subtask_info)
    ...
    def _execute_navigation_step(self, parsed_xml, encoded_xml, current_page_index):
        ...
        if ui_index is not None:
            ...
            return self._create_click_action(parsed_xml, encoded_xml, ui_index, current_page_index)
    ...
    def _create_click_action(self, parsed_xml, encoded_xml, ui_index, page_index):
        """클릭 액션 생성 및 추적 변수 설정"""
    ...
    def mark_last_action_explored(self, end_page: int = -1):
        ...
        page_data = self.memory.page_db.loc[self.memory.page_db['index'] == self.last_explored_page_index]
        if page_data.empty:
        ...
        if end_page != -1 and end_page != self.last_explored_page_index and found_subtask_name:
            if self.last_explored_page_index not in self.page_graph:
                self.page_graph[self.last_explored_page_index] = []

            # 중복 edge 체크 (동일한 subtask로 가는 edge가 이미 있는지 확인)
            existing_edges = [(p, s) for p, s in self.page_graph[self.last_explored_page_index]]

            if (end_page, found_subtask_name) not in existing_edges:
                self.page_graph[self.last_explored_page_index].append((end_page, found_subtask_name))
                log(f"Page graph updated: {self.last_explored_page_index} -> {end_page} via '{found_subtask_name}'", "blue")
        ...
        if hasattr(self, 'exploration_algorithm') and self.exploration_algorithm in ["GREEDY_BFS", "GREEDY_DFS"] and end_page != -1:
            self._load_page_unexplored_subtasks(end_page)

        # STEP 5: 서브태스크를 explored로 마킹 + 액션 저장
        if found_subtask_name:
            ...
            log(f"Marked subtask '{found_subtask_name}' (trigger_ui={trigger_ui_index}) as explored and saved action on page {self.last_explored_page_index} -> {end_page}", "green")

        # STEP 6: 추적 변수 초기화
        self._reset_explore_tracking()

    def _load_page_unexplored_subtasks(self, page_index: int):
        """특정 페이지의 unexplored 서브태스크 로드 (모든 trigger UI별로)"""
        page_path = os.path.join(self.memory.page_database_path, str(page_index))
        available_file = os.path.join(page_path, "available_subtasks.csv")
        ...
        if os.path.exists(available_file):
            try:
                ...
                if not unexplored_df.empty:
                    ...
                    if subtask_infos:
                        self.unexplored_subtasks[page_index] = subtask_infos
                        log(f"Loaded {len(subtask_infos)} unexplored subtask-trigger combinations for page {page_index}", "blue")
            except Exception as e:
                log(f"Error reading available_subtasks.csv for page {page_index}: {e}", "yellow")
    ...
    def _find_nearest_unexplored_info(self, current_page: int) -> tuple:
        ...
        # 현재 페이지에 unexplored가 있으면 바로 반환
        if current_page in self.unexplored_subtasks and self.unexplored_subtasks[current_page]:
            subtask_info = self.unexplored_subtasks[current_page][0]
            return (current_page, subtask_info, [])
        ...
        while queue:
            ...
            for next_page, subtask_name in self.page_graph[page]:
                ...
                # unexplored 서브태스크가 있는 페이지 발견
                if next_page in self.unexplored_subtasks and self.unexplored_subtasks[next_page]:
                    target_subtask_info = self.unexplored_subtasks[next_page][0]
                    return (next_page, target_subtask_info, new_path)
    ...
    def _find_deepest_unexplored_info(self, current_page: int) -> tuple:
            ...
            for next_page, subtask_name in self.page_graph[page]:
                ...
                if next_page in self.unexplored_subtasks and self.unexplored_subtasks[next_page]:
                    ...
                    if new_depth > max_depth:
                        max_depth = new_depth
                        target_subtask_info = self.unexplored_subtasks[next_page][0]
                        deepest_result = (next_page, target_subtask_info, new_path)
