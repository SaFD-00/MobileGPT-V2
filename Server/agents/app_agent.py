...
from utils.utils import get_openai_embedding, cosine_similarity, query, safe_literal_eval
...
def get_package_info(package_name):
    ...
    if "about_this_app" in search_result and "snippet" in search_result["about_this_app"]:
        ...
        logger.error("No description: " + package_name)
...
class AppAgent:
    ...
    def update_app_list(self, new_packages):
        ...
        for package_name in new_packages:
            if not package_name in known_packages:
                ...
                logger.info("New app detected: " + package_name)

        self.database.to_csv(self.database_path, index=False)
        logger.info(f"App Analyze Finished. Number of stored applications: {len(self.database)}")
    ...
    def predict_app(self, instruction) -> str:
        ...
        logger.info(f"predicted App: {response['app']}")
