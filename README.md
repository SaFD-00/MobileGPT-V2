...
MobileGPT-V2는 다중 에이전트 아키텍처를 통해 Android 기기에서 복잡한 태스크를 수행하는 지능형 자동화 시스템입니다. 기존 MobileGPT 연구를 기반으로, 자율적인 UI 탐색과 학습이 가능한 **Auto-Explore** 모듈을 도입하여 앱 지식 습득에 필요한 수동 작업을 크게 줄였습니다.
...
3. **생성 (Generates)**: M3A 스타일 액션 설명 (각 액션 후 변경된 내용)
4. **작성 (Creates)**: UICompass 스타일 페이지 요약 (페이지가 보여주고 허용하는 것)
...
| **Plan** | Mobile Map을 사용하여 최적 경로 생성 → `verify_plan()` | PlannerAgent, StepVerifyAgent |
...
| **Plan** | Mobile Map 경로 계획 (UICompass 스타일) | PlannerAgent |
...
| **HistoryAgent** | M3A 스타일 액션 설명, 시맨틱 가이던스 생성 |
| **SummaryAgent** | UICompass 스타일 페이지 요약 생성 |
| **FilterAgent** | 지시어 관련성을 위한 서브태스크 필터링 (4-Step 워크플로우) |
| **PlannerAgent** | 목표 분석, Mobile Map 경로 계획 |
...
| step | 액션 시퀀스 번호 |
| action | 액션 파라미터 (JSON) |
| **description** | M3A 스타일: 액션 후 변경 내용 |
| **guidance** | 액션의 시맨틱 의미 |
...
- **LangGraph**: LLM을 사용한 상태 기반 다중 액터 애플리케이션 구축 프레임워크
