...
5. [실행 방법](#5-실행-방법)
   - [5.1 서버 실행](#51-서버-실행)
   - [5.2 기본 모드](#52-기본-모드-사용자-명령-실행)
   - [5.3 추론 모드](#53-추론-모드-inference)
   - [5.4 수동 탐색 모드](#54-수동-탐색-모드)
   - [5.5 자동 탐색 모드](#55-자동-탐색-모드)
6. [메모리 구조](#6-메모리-구조)
7. [벤치마크 데이터셋](#7-벤치마크-데이터셋)
8. [아키텍처](#8-아키텍처)
9. [주의사항 및 라이선스](#9-주의사항-및-라이선스)
...
| **LangGraph 추론** | 자동 subtask 선택 및 검증 (inference 모드) |
...
| **VerifyAgent** | 다음 화면 검증 (추론 모드) | 선택된 subtask | 간다/가면 안된다 판정 |
...
os.environ["VERIFY_AGENT_GPT_VERSION"] = "gpt-5.2-chat-latest"       # 추론 모드: 다음 화면 검증
```
...
# 추론 모드: LangGraph 기반 자동 subtask 선택/검증 (기본값)
python main.py --mode inference
```

| 모드 | 설명 | 서버 클래스 |
|------|------|------------|
| `task` | 사용자 명령 실행 | `Server` |
| `explore` | 수동 화면 탐색 | `Explorer` |
| `auto_explore` | 자동 UI 탐색 | `AutoExplorer` |
| `inference` | LangGraph 추론 (기본값) | `InferenceServer` |
...
# 모드 지정 실행
python ./main.py --mode inference    # LangGraph 추론 (기본값)
python ./main.py --mode task         # 사용자 명령 실행
...
# 출력 예시:
# InferenceServer listening on 192.168.0.100:12345
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `--mode` | `inference` | 서버 모드 (`task`, `explore`, `auto_explore`, `inference`) |
...
│                                                                │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐        │
│  │ MemoryAgent │ → │ SelectAgent │ → │ VerifyAgent │        │
...
- **자동 검증**: VerifyAgent가 선택한 subtask의 다음 화면을 분석하여 "가면 안된다" / "간다" 판정
- **재선택 루프**: 검증 실패 시 해당 subtask를 거부 목록에 추가하고 재선택 (최대 5회)
- **상태 관리**: LangGraph State로 세션, subtask, 검증 상태 관리
...
- 디렉토리 구조
- 핵심 클래스 (MobileGPT, Server, Explorer, AutoExplorer, **InferenceServer**)
- 에이전트 파이프라인 상세
- **LangGraph 추론 그래프** (InferenceState, 노드별 역할)
- 메모리 시스템 클래스 다이어그램
